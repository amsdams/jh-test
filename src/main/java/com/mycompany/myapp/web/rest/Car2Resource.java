...
public class Car2Resource {
    ...
    /**
     * SEARCH  /_search/car-2-s?query=:query : search for the car2 corresponding
     * to the query.
     *
     * @param query the query of the car2 search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/car-2-s")
    @Timed
    public ResponseEntity<List<Car2DTO>> searchCar2S(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of Car2S for query {}", query);
        Page<Car2DTO> page = car2Service.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/car-2-s");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
