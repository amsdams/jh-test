...
public class Owner4Resource {
    ...
    /**
     * SEARCH  /_search/owner-4-s?query=:query : search for the owner4 corresponding
     * to the query.
     *
     * @param query the query of the owner4 search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/owner-4-s")
    @Timed
    public ResponseEntity<List<Owner4DTO>> searchOwner4S(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of Owner4S for query {}", query);
        Page<Owner4DTO> page = owner4Service.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/owner-4-s");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
