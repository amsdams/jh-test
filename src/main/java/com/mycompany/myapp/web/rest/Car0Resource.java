...
public class Car0Resource {
    ...
    /**
     * SEARCH  /_search/car-0-s?query=:query : search for the car0 corresponding
     * to the query.
     *
     * @param query the query of the car0 search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/car-0-s")
    @Timed
    public ResponseEntity<List<Car0DTO>> searchCar0S(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of Car0S for query {}", query);
        Page<Car0DTO> page = car0Service.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/car-0-s");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
